...
> Integration tests require all three databases running locally. Use dedicated test database names (e.g. `fastchat_test`) so tests never affect development data.
...
├── factories/
│   └── user.factory.js
└── setup.js              # jest.mock for logger and Socket.io
```
...
// Logger (suppress output during test runs)
...
- Socket.io is mocked globally in `tests/setup.js`:
...
This prevents the need for an actual WebSocket server during HTTP tests.
